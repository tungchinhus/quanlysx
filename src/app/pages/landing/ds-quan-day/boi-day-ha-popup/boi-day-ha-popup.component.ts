...
export interface BoiDayHaData {
  ...
  ghi_chu?: string;
  trang_thai: number;
  // Thêm các field đặc biệt cho bối dây hạ
...
// Interface mới cho API save-bd-ha theo specification
...
export class BoiDayHaPopupComponent implements OnInit {
  ...
  onFormValueChange() {
    ...
    console.log('Form validation status:', {
      ...
      formValid: this.boiDayHaForm.valid,
      requiredFields: {
        quy_cach_day: this.boiDayHaForm.get('quy_cach_day')?.valid,
        so_soi_day: this.boiDayHaForm.get('so_soi_day')?.valid,
        nha_san_xuat: this.boiDayHaForm.get('nha_san_xuat')?.valid,
        ngay_san_xuat: this.boiDayHaForm.get('ngay_san_xuat')?.valid
      }
    });
  }
  ...
  private logSubmitData(data: BoiDayHaSubmitData | BoiDayHaApiRequest): void {
    ...
    if (isSubmitData(data)) {
      console.log('Data type: BoiDayHaSubmitData');
      ...
      console.log('Data type: BoiDayHaApiRequest');
      console.log('Key fields:', {
  ...
  onSubmit(): void {
    ...
    this.http.post<any>(`${this.commonService.getServerAPIURL()}api/ProductionData/save-bd-ha`, apiRequest, {
  ...
  private mapToApiRequest(formData: any, quanDayData: any): BoiDayHaApiRequest {
    ...
    // Map sang API format
    const apiRequest: BoiDayHaApiRequest = {
      masothe_bd_ha: quanDayData.kyhieuquanday || 'BDH_' + Date.now(), // Mã số thẻ bối dây hạ
  ...
  hasUnsavedChanges(): boolean {
    return this.boiDayHaForm.dirty || this.boiDayHaForm.touched;
  }
  ...
  resetForm() {
    ...
    this.boiDayHaForm.markAsUntouched();
    this.boiDayHaForm.markAsPristine();
